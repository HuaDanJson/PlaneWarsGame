...
public class GamePause {
	...
	public GamePause(Bitmap bmpPause_bg,Bitmap bmpPause_back,Bitmap bmpPause_continue,Bitmap bmpPause_exit) {
		...
		btn_continue_x = MyView.screenW / 2 - bmpPause_continue.getWidth() / 2;
		btn_continue_y = MyView.screenH *2/5- bmpPause_continue.getHeight();

		btnX = MyView.screenW / 2 - bmpButton.getWidth() / 2;
		btnY = MyView.screenH *4/6- bmpButton.getHeight();


		btn_exit_x = MyView.screenW / 2 - bmpPause_exit.getWidth() / 2;
		btn_exit_y = MyView.screenH*5/6- bmpPause_exit.getHeight();
			...
			if (pointX > btnX && pointX < btnX + bmpButton.getWidth()) {
				if (pointY > btnY && pointY < btnY + bmpButton.getHeight()) {
					...
					if(MyView.soundFlag){
						...
						MyView.mediaPlayer2.pause();
						MyView.mediaPlayer.start();
					}
					MyView.Pause_flag = 1;
					MyView.gameState = MyView.GAME_MENU;
				}
			}
		}
